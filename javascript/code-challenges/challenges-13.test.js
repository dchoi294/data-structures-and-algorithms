...
// Solution code here...
  return arr.length ? arr.reduce((shortIdx, str, idx) => str.length > arr[shortIdx].length ? idx: shortIdx, 0) : -1;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(numb => numb.replace(/\(|\)|-| /g,''));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((letter,idx) => idx%2).join('');
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.reduce((tf, str) => str.includes(':)') ? tf : tf = false, true);
};
